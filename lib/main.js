...
const CONTEXT_MENU = "contentAreaContextMenu";  //  "popuphidden"
...
//      thestr2 = locale.get(WINDOW_TYPE.replace(":", "-"));
		thewin = parent.openDialog( "chrome://targetout/content/iframe.xul",  //  The chrome URL
...
//              else thewin.sessionStorage.removeItem(LIB_ISBN);
    ...
    //          if (thestr.startsWith('about:addons'))
                ...
                thecfg.contentScriptOptions = {
                    "target": thehash, "prior4sel": thepref,
...
var shadowBtn = {   isbn: LIB_ISBN,
    ...
    handleEvent : function(eva) {
        ...
        let thestr, thebro = eva.currentTarget;
//      if (eva.target === thebro) thebro = getMostRecentBrowserWindow();
        ...
        //  case: if not current or not top of "pageshow" event
            ...
            thestr = (content.documentURI || "about:").toLowerCase();
    ...
    //        thebro.console.log(disabled, content.documentURI);
        ...
        } else if (thehash) { // menu event
            docum = (thebro.ownerDocument || {}).documentElement;
            thestr = [ "#", ":hover" ].join(this.isbn);
            if (docum)
                if (!(docum.querySelector(thestr))) 
                    thehash = "";
//  dump("\n_dvk_dbg_, menu event: "); dump(docum.querySelector(thestr)); dump("\n");
        }
        return;
    }
}   //  var shadowBtn
...
function sdkFireStyle(awin) {
    ...
    //  frame wait for popup of btn body
        ...
        let themenu = thewin.document.getElementById(CONTEXT_MENU);
        if (themenu) themenu.addEventListener("popuphidden", shadowBtn, false);
...
function attachContent() {
    ...
    thecontent = pageMod.PageMod({ include: "*",
        ...
        contentScriptOptions: {
            "tagslist": preferences.prefs["boxabletags"] },
        onAttach: function(worker) { thehash = "" },
        onMessage: function(anhash) { thehash = anhash || "" }
    })
}   //  function attachContent()
    ...
    try {
        ...
        if (preferences.prefs["click2hash"]) {
            thewin.setTimeout( function(adoc) {
                ...
                if (thestyle) shadowBtn.isbn = 
                    thestyle.title || shadowBtn.isbn;
    thewin.console.log("_dvk_dbg_, shadow btn of main: ", shadowBtn.isbn);
            }, 1, thewin.document );
            attachContent();
        }
    ...
    windows(BROWSER_TYPE).forEach(
        function(awin) {
            try {
                ...
                let themenu = awin.document.getElementById(CONTEXT_MENU);
                if (themenu) themenu.removeEventListener(
                                "popuphidden", shadowBtn, false );
