...
if (thebody) (function() {
    ...
    function activeTest(astr) {
		if (!((astr || {}).length)) return false;
		var tagslist = self.options.tagslist || "table div p article";
			tagslist = tagslist.trim().replace(/\s+/g, " ").split(" ").
				map( function(atag) { return ":".concat(atag || "") } );
		return tagslist.some( function(atag) { 
                    return this.endsWith(atag)
                }, ":".concat(astr.toLowerCase()) );
    }
 ...
 const FRAMES_LIST = [ ":frame", ":FRAME", ":iframe", ":IFRAME" ];
    ...
    var selection = false;
    var thehash = self.options.target || "?";
    var thenode = document.getElementById(thehash);
    if (!thenode) {
...
//	dump("\n_dvk_dbg_, original node: "); dump(tagName); dump("\n");
	...
	if(!theframe) {
        ...
        //	NOTE: first-of-type body will be more accurate?
            ...
            for (newnode of docum.querySelectorAll(EXCLUDE_TAGS))
